---
title: "Using csdrtools to import one or multiple FlexFiles"
...
One of the core functions of `csdrtools` is to import the FlexFile into the R environment. `csdrtools` supports importing both one FlexFile or a folder of multiple FlexFiles into R. The functions:
...
This vignette demonstrates how to import the FlexFile using `csdrtools`.

## Importing one FlexFile

To import one FlexFile, first point `read_ff` to a .zip of JSON tables of a FlexFile.

```{r echo = TRUE, warning=FALSE, messages=FALSE, eval = T}
...
`csdrtools` also provides functionality for the Quantity Data Report (QDR). The workflow for reading in the QDR is the same as for the FlexFile (and uses many of the same functions). However, instead of `flatten_ff`, use `flatten_qdr`.

```{r echo = TRUE, warning=FALSE, messages=FALSE, eval = T}
quantity_data <- system.file("extdata", "Sample File_Q.zip", package = "csdrtools")
