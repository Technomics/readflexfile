...
We present an alternative approach to handling cost data, specifically CCDRs. This paper describes that alternative – the **`readflexfile`** and **`ezflexfile`** packages within R, an existing open source data analytics framework. 

As the first adopters of the FlexFile, we rely on tools such as R. Soon, the cost community will, too. We strongly believe that the community will not only benefit from, but in fact rely upon packages such as **`readflexfile`** and **`ezflexfile`** as it transitions into an age of bigger and more complex datasets.
...
To achieve this, we’ve created a collection of R packages which handle the tasks of importing, tidying, transforming, and visualizing the FlexFile. **`readflexfile`** and **`ezflexfile`** contain the following core functions:
...
*	The `plot_area()`, `plot_bar()`, `plot_scatter()`, `plot_sunburst()`, and `plot_sankey()` functions **visualize** the FlexFile with pre-built graphics.

The **`readflexfile`** (imports and tidies the FlexFile) and **`ezflexfile`** (which produces visualizations and summary tables) packages are built under the tidyverse philosophy. Functions are intended to be strung together with the pipe (`%>%`) operator. This represents a powerful benefit of the tidyverse. Pipes allow the analyst to clearly express a sequence of multiple operations, in which the output of the prior operation is fed into the input of the next operation. The functions within **`readflexfile`** and **`ezflexfile`** support this capability.

The **`readflexfile`** and **`ezflexfile`** packages are also designed to interact with other tidyverse packages, such as `dplyr`. The `dplyr` package is the tidyverse standard for data manipulation, such as filtering and subsetting data, and transforming variables. This will be discussed in more detail in section 1.6.2: Tidy.
...
The **`readflexfile`** and **`ezflexfile`** packages solve this problem. This package was not developed as a “science project”, but as a need-driven solution to eliminate the barrier to entry for using the FlexFile.

The following sections describe each function within **`readflexfile`** and **`ezflexfile`** in more detail. This information is also contained in each function’s help file.^[ Help files in R can be accessed with the ? followed by the function name. For example, `?read_ff` will load the help file for `read_ff()`.]
...
R has a number of functions available to import data, typically prefixed with the word “read” (e.g., `read_csv()`, `read_excel()`, `read_json()`). Sticking with this convention, **`read_flexfile`** provides `read_ff()` and `read_folder()` to read the FlexFile data into R.
...
`read_ff(file, .show_check = FALSE)`

* `file`: Path to a FlexFile .zip archive.
* `.show_check`: Logical whether to print information to the console about the file check to the console or not.
...
`read_folder(folder, read_function, .clean_file_names = TRUE, .id = NULL, .recursive = TRUE, ...)`
...
* `recursive`: Logical to recursively load the folder.
...
folder <- system.file("extdata", package = "readflexfile")
...
* `.data`: A list (or list of lists) of data frames.
* `var = "doc_id"`: Character name of which to name the ID column. The default is `doc_id`.
...
Doing this for every FlexFile is a significant resource strain. The `flatten_ff()` function, and **`readflexfile`** and **`ezflexfile`** more generally, provides an instant, repeatable solution to these tasks. This saves time organizing data, and provides more for analysis.
...
* `.id = "doc_id"`: The name of the ID column which uniquely identifies a FlexFile.
...
DD_1921 <- tbl_1921(ff_flat, unit = "dollars")
...
DD_1921_1 <- tbl_1921_1(ff_flat, unit = "dollars")
...
Well-conceived visualizations serve many purposes in Cost. Hierarchical relationships, trend analysis, and comparative visuals help an analyst better understand the data set, and communicate the data set to decision makers. The **`ezflexfile`** package contains several plotting functions. The `plot_area()` function returns different area charts, while `plot_bar()` returns bar charts. Much the same, `plot_scatter()` return several scatterplots. The `plot_sankey()` and `plot_sunburst()` functions return Sankey diagrams and sunburst charts, respectively, useful for visualizing relationships. The following sections describe these two functions.

#### `plot_area()` and `plot_bar()`

The `plot_area()` and `plot_bar()` functions deliver a set of visualizations of the FlexFile. The functions return different graphics based on the arguments described below. 

##### Usage and Arguments

`plot_area(flat_flexfile, type = 1, unit = c("dollars", "percent"))`
`plot_bar(flat_flexfile, type = 1, unit = c("dollars", "percent"))`

* `flat_flexfile`: A flattened flexfile created from the `flatten_ff()` function.

* `type`: A plot type from plot_area_types(). Can be either the integer index or the character type.

* `unit`: Either "dollars" or "percent".

##### Example 

##### `plot_area(type = 1)`

Time-phased dollars and hours are an important aspect of the FlexFile. A natural way to view time phasing is through an area (or sand) chart, where the x-axis represents the date costs are incurred and the y-value depicts those costs. `plot_area(type = 1)` presents costs over time, colored by nonrecurring (NR) or recurring (R) effort. This helps readily distinguish the ratio of NR to R costs.

```{r eval=T, echo=TRUE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
ff_flat <- ff_flat %>%
  dplyr::filter(doc_id == 1)

ff_flat %>%
  plot_area(type = 1, unit = "dollars")
...
The `plot_bar(type = 2)` function uses the same arguments as `plot_area(type = 1)`, but instead presents a bar chart of WBS elements colored by standard category. This provides details into the composition of each WBS element.
...
ff_flat_hardware %>%
  plot_bar(type = 2, unit = "hours")

```

##### `plot_bar(type = 4)`

The functional category field in the FlexFile lends insight into contractor’s internal direct and overhead pricing categories. The `plot_bar(type = 4)` function offers an area chart WBS elements broken out by direct and overhead labor.

```{r echo = TRUE, eval = T, fig.width=7,fig.height=5}

ff_flat %>%
  plot_bar(type = 4, unit = "percent")
...
`plot_sankey(flattened_ff, type)`

* `flattened_ff`: Data frame of a flattened FlexFile.
...
The `plot_sankey(type = 1)` function illustrates the flow of costs from a parent WBS element to a contractor-native account. This example demonstrates this relationship for the Engineering standard category.
...
The `plot_sankey(type = 2)` function outputs the same plot as `plot_sankey(type = 1)`, but for the functional category ot standard category relationship. Filtering to one WBS element, in this case WBS element Subsystem 1.1.5.3, the funcitonal categories that map to the respective standard categories are visualized from highest to lowest cost, denoted by the size of the node. When outputted in R, this visual leverages an interactive “tool-tip” that displays costs and relationship details as one hovers over the diagram. 
...
The **`readflexfile`** and **`ezflexfile`** packages provide a set of functions that serve as building blocks for powerful analysis of the FlexFile in R. Each step in the process outlined above – import, tidy, transform – is necessary before any analysis or visualization is possible.

R provides the necessary environment for **`readflexfile`** and **`ezflexfile`**. By developing the functions outlined in this paper, we have created an instantly scalable and repeatable process, capable of processing one FlexFile, or 100. This is simply not possible with Excel. Therefore, as the cost community transitions into the big data age, it is necessary that capabilities such as **`readflexfile`** and **`ezflexfile`** exist and are adopted.
...
The FlexFile in its current state is difficult to use within Excel. The growing user base of tools such as R within the cost community is a critical enabler to using larger data sources. As the use of the FlexFile grows, so too will the need for a solution to provide faster, more accurate, and more insightful analysis. **`readflexfile`** and **`ezflexfile`** provide this solution.

The FlexFile is still largely in its infancy, with only a portion of programs submitting their first FlexFiles to CADE. We envision the **`readflexfile`** and **`ezflexfile`** packages evolving with the FlexFile. This not only emphasizes the flexibility of **`readflexfile`** and **`ezflexfile`**, but also the scalability of cost analysis in R.
