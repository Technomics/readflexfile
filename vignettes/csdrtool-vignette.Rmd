...
Applying `listindex_to_col()` to the list of FlexFiles (“flexfiles”) adds a column, `ff_id`, to each table with the list index of each FlexFile.

```{r echo = TRUE}
flexfiles <- flexfiles %>%
  listindex_to_col(var = "ff_id")

# "ff_id" == 1 added to each table in the first indexed list.
...
ff_flat_accounts <- ff_flat %>%
  select(ff_id, account_name, value_dollars, value_hours)
...
DD_1921_1_actuals <- DD_1921_1 %>%
  dplyr::select(standard_category, FAC_NONRECURRING, FAC_RECURRING)
...
head(tbl_direct_rates(ff_flat)) %>%
  dplyr::select(standard_category, functional_category_name, direct_rate)
...
ff_flat_engineering <- ff_flat %>%
  filter(stringr::str_detect(standard_category, 'ENGINEERING'))
